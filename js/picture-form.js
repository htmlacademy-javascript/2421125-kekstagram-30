...
const picturePreviewElement = pictureFormElement.querySelector(
  '.img-upload__preview img'
);
const effectsPreviewElement =
  pictureFormElement.querySelectorAll('.effects__preview');
...
const isHashtagValid = (value) =>
  normalizeTags(value).every((tag) => REG_EXP.test(tag));
...
const validateHashtagsLength = (value) =>
  normalizeTags(value).length <= HASHTAG_LENGTH_COUNT;

const validateComment = (comment) =>
  comment.trim().length <= COMMENT_LENGTH_COUNT;
