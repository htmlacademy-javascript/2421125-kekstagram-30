...
const scaleControlSmallerElement = pictureFormElement.querySelector(
  '.scale__control--smaller'
);
const scaleControlBiggerElement = pictureFormElement.querySelector(
  '.scale__control--bigger'
);
const scaleControlValueElement = pictureFormElement.querySelector(
  '.scale__control--value'
);
const imagePreviewElement = pictureFormElement.querySelector(
...
const onSmallerButtonClick = () => {
  const value = Math.max(
    parseInt(scaleControlValueElement.value, 10) - SCALE_STEP_COUNT,
    SCALE_MIN
  );
  scalePicture(value);
};
